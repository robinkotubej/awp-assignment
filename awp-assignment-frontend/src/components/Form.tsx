...
const Form = ({ type, questionId, setQuestion }: Props) => {
  const [username, setUsername] = useState<string>('')
  const [userMessage, setUserMessage] = useState<string>('')
  const [message, setMessage] = useState<{
    message: string
    error?: boolean
  }>({ message: `Post your ${type}` })

  const handlePost = async (event: FormEvent) => {
    event.preventDefault()
    let data
    if (type === 'answer' && questionId && setQuestion) {
      data = await Api.postAnswer(username, userMessage, questionId)
      setQuestion(data)
    } else {
      data = await Api.postQuestion(username, userMessage)
    }
    setMessage(data)
    console.log(data)
  }
  ...
  return (
    <Container>
      ...
      <StyledForm onSubmit={event => handlePost(event)}>
        ...
        <TextInput
          rows={5}
          placeholder={`${type === 'answer' ? 'Answer:' : 'Question:'}`}
          value={userMessage}
